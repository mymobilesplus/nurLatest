import React, { Component } from 'react';
import { Text,View,StyleSheet, FlatList, RefreshControl, BackHandler, StatusBar, ScrollView,TouchableOpacity, Image, ImageBackground } from 'react-native';

import { Container, Header, Title, Body, Content, Card,CardItem, Left, Icon, Right, Button, Row, Col} from 'native-base';

import { MenuProvider } from 'react-native-popup-menu'
import VaccinationComponent from '../../Dashboard/components/Vaccinationcomponent';
class Upcoming extends Component {
    
    constructor(props) {
        super(props);
        this.state = {
            products:[{
                name: 'Here comes the description of the medical records and it could be display on two lines...',
                date:'June 30, 2020',
                id: 1
            },
            {
                name: 'Here comes the description of the medical records and it could be display on two lines...',
                date:'June 30,2020',
            },
            {
                name: 'Here comes the description of the medical records and it could be display on two lines...',
                date:'June 30,2020',
            },
            
        ]
        }
       
    } 
    
   
    renderRow({item, index}){
        return(
            <TouchableOpacity  style={{flex: 1}} >
                 <MenuProvider >
                <Row style={{marginTop:10, marginBottom:10,}}>
                    <Col style={{marginLeft:10,}}>
                        <Row>
                            <Text style={{color:'#000', marginTop:20, fontWeight:'bold'}}>{item.name}</Text>
                        </Row>
                        <Row>
                            <Text style={{color:'#aaa', marginTop:20, fontWeight:'bold'}}>Vaccination Date: {item.date}</Text>
                        </Row>
                    </Col>
                    <Col style={{alignItems:'flex-end'}}>
                    <VaccinationComponent/>
                    </Col>
                </Row>
                </MenuProvider>
            </TouchableOpacity>
        )
    }

    render() {
        return (
           <Container>
                <FlatList 
                data={this.state.products} 
                renderItem={this.renderRow}
                />
            </Container>
        );
    }
}

const styles = StyleSheet.create({
    container: {
      flex: 1,
      marginBottom: 50
    },
    itemText: {
        fontSize: 16, 
        color: '#000', 
    },
    itemText1: {
        fontSize: 13, 
        color: '#aaa',
    },

    itemText2: {
        fontSize: 16, 
        color: '#aa0114', 
    }, 
    itemText3: {
        fontSize: 16, 
        color: '#aaa', 
        textDecorationLine: 'line-through'
    },   
     backgroundTranslucent:{
        backgroundColor: "rgba(255,255,255, 0.4)"
    }
  });

export default Upcoming;
