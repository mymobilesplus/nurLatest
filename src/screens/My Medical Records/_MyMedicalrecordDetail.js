...
// import {List} from "../../context/MedicalRecordList"
...
function _MyMedicalrecordDetail(props) {
    ...
    const getRecord = async()=>{
        if(props.route.params.record!=null){
            setLoading(true)
            let r = JSON.parse(props.route.params.record);
            ...
            setFile(res.medicalRecord.files)
            setRecord(res.medicalRecord);
            setLoading(false)
            ...
            <Container>
                ...
                {!loading &&
                    <Content>
                            ...
                            <Col style={{alignItems:'flex-end',flex:0.1}}>
                                <Popover
                                    ...
                                    onBackdropPress={() => setVisible(false)}>
                                        <Layout style={{alignItems:"flex-start", width:180}}>
                                            <View style={{borderBottomWidth:0.5,width:180,borderBottomColor:"#8A92A3"}}>
                                            <Text onPress={()=>{setVisible(false); props.navigation.navigate('Medical Record Form', {medicalRecord: record})}} style={{padding: 10, fontSize: 16, color:"#596377"}}>
                        ...
                        <Row style={{padding:25, borderBottomWidth:1, borderBottomColor:'#ccc'}}>
                            <Col>
                                <Text style={{fontSize:15}}>File Type: {record.medicalRecordType}</Text>
                            </Col>
                        </Row> 
...
export default connect(mapStateToProps,mapDispatchToProps)(_MyMedicalrecordDetail)
