    ...
    return (
        <View style={Styles.mainContainer}>
           <View style={Styles.addNewMemberContainer}> 
                <Text onPress={()=>props.navigation.navigate('Medical Record Form', {familyMember: ''})} style={{color:theme.colors.primary, textAlign: 'right', width: '100%', marginRight: 20, marginBottom:30}}>Skip</Text>
                <View style={{flex:0,width:wp('75%')}}>
                    <Text style={Styles.topText}>Whom do you want to assign this medical record to?</Text>
                </View>
                
                <Button style={Styles.button} size="large" onPress={()=>navigation.navigate('AddPhotoRegistration', {returnURL: 'Add New Member'})} >Add New Member</Button>
           </View>
           <View style={Styles.memberContainer}>
           {List.length > 0 &&
                <FlatList
                    data={List}
                    renderItem={({item, index})=>renderFamilyMembers({item, index, props, Styles, navigate: ()=>props.navigation.navigate('Medical Record Form', {familyMember: item})})}
                    refreshControl={
